		...
		if (msg.message == WM_LBUTTONDOWN)
		{
			...
			if (msg.x >= 545 && msg.x <= 690 && msg.y >= 555 && msg.y <= 610)
			{
				...
				if(a.spacex > 24)
				{
					...
					MessageBox(hWnd, L"行数超限", L"参数错误", MB_OK | MB_ICONINFORMATION);
				}
				else if (a.spacey > 30)
				{
					HWND hWnd = GetForegroundWindow();
					MessageBox(hWnd, L"列数超限", L"参数错误", MB_OK | MB_ICONINFORMATION);
				}
				else if (a.mine > a.spacex * a.spacey * 75 / 100)
				{
					HWND hWnd = GetForegroundWindow();
					MessageBox(hWnd, L"雷数超限", L"参数错误", MB_OK | MB_ICONINFORMATION);
				}
				else
				{
					return a;
